    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow flex flex-col">
      <div className="relative h-48">
        ...
        {item.status === ItemStatus.AVAILABLE && (
          <div className="absolute top-2 right-2 bg-green-500 text-white px-2 py-0.5 rounded text-xs font-medium uppercase">
            Available
          </div>
        )}
        {item.status === ItemStatus.PENDING && (
          <div className="absolute top-2 right-2 bg-yellow-500 text-white px-2 py-0.5 rounded text-xs font-medium uppercase">
            Pending
          </div>
        )}
      </div>
      <div className="p-4 flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between text-xs text-gray-500 mb-4">
           ...
           <span className="flex items-center gap-1">Best by: {item.expiryDate}</span>
        </div>
