...
interface MessagesPageProps {
  ...
  onSubmitClaimerFeedback: (messageId: string, feedback: {
    responseTime: number;
...
export default function MessagesPage({ 
  ...
  const [showClaimerFeedbackModal, setShowClaimerFeedbackModal] = useState(false);
      ...
      {!selectedMessage ? (
        <div className="messages-list">
          <div className="messages-header">
            ...
            <h2>Messages</h2>
            <span className="unread-badge">{messages.filter(m => m.unreadCount > 0).length} unread</span>
          </div>
              ...
              >
                ...
                <div className="message-item-content">
                  ...
                  <div className="message-item-info">
                    <img src={message.item.image} alt="" className="item-thumb" />
        ...
        <div className="chat-view">
          <div className="chat-header">
            <button className="back-btn" onClick={() => setSelectedMessageId(null)}>
              <ArrowLeft size={20} />
            </button>
            <div className="chat-header-info">
              <img src={selectedMessage.otherUser.avatar} alt={selectedMessage.otherUser.name} />
              <div>
                <h3>{selectedMessage.otherUser.name}</h3>
                <p>Usually responds in {selectedMessage.otherUser.responseTime}</p>
              </div>
            </div>
            <div className="chat-header-actions">
              {selectedMessage.isOwner && selectedMessage.status === 'pending' && (
                <button 
